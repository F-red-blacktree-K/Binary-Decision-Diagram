        ...
        else
        {
            ...
            while (!file.eof())
            {
                ...
                if (fileline == ".i")
                {
                    ...
                    //賦值範圍: pow(2, 變數數量-1) - 1  節點數量: pow(2, 變數數量) - 1
                    node_num = pow(2, variables_num) - 1;
                    assign_index = pow(2, variables_num - 1) - 1;

                    for (int index = 1; index <= assign_index; index++)
                    {
                ...
                else if (fileline == ".p")
                {
                    ...
                    for (int i = 0; i < product_term_nums; i++)
                    {
                        ...
                        for (int j = 0; j < bit_table.size(); j++)
                        {
                            ...
                            //如果都一樣compare沒有變成fasle 表示比對成功，則該格為1
                            if (compare)
                            {
                                // then else 切換
                                if (j % 2 == 0)
                                    else_edge[table_index] = node_num + 1;
                                else if (j % 2 == 1)
                                    then_edge[table_index] = node_num + 1;
                            }
                        }
                ...
                for (int i = 1; i <= node_num; i++)
                {
                    for (int j = i + 1; j <= node_num; j++)
                    {
                        ...
                        if (else_edge[i] == else_edge[j] && then_edge[i] == then_edge[j] && state[j] == 'n' && ilb[i] == ilb[j])
                        {
                            check = true;
                            state[j] = 'r';
                            //找出tabel中被redundent掉的節點將其改成保留節點
